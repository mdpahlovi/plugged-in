...
const DeleteConfirmModal = ({ deletingRecordId, setDeletingRecordId, refetch }) => {
    ...
    return (
        <Transition appear show={deletingRecordId ? true : false} as={Fragment}>
            <Dialog as="div" className="relative z-10" onClose={() => setDeletingRecordId(null)}>
                <Transition.Child
                    as={Fragment}
                        ...
                        >
                            <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-base-100 p-6 space-y-4 align-middle shadow-xl transition-all">
                                ...
                                <div className="mx-auto w-3/4 grid grid-cols-2 gap-4">
                                    <Button onClick={() => handleYes(deletingRecordId)}>Yes</Button>
                                    <ButtonOutline onClick={() => setDeletingRecordId(null)}>No</ButtonOutline>
                                </div>
                            </Dialog.Panel>
                        </Transition.Child>
                    </div>
