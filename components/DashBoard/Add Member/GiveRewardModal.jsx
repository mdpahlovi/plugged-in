...
const GiveRewardModal = ({ setSelectedMemberTask, selectedMemberTask, team }) => {
        ...
        <div>
            ...
            <div className="modal">
                <div className="modal-box rounded-lg relative">
                    <label onClick={() => setSelectedMemberTask(null)} htmlFor="giveRewardModal" className="btn btn-sm btn-circle absolute right-2 top-2">
                        ✕
                    </label>
                    {tasks?.map((task) => (
                        <TaskCollapse tasksRefetch={tasksRefetch} team={team} key={task?._id} task={task} />
                    ))}
                </div>
            </div>
        </div>
