...
const TaskCollapse = ({ task, tasksRefetch, team }) => {
    const { data: media = {} } = useQuery({
        queryKey: [task, "media"],
        queryFn: () => fetch(`https://plugged-in-server.onrender.com/media/${task?.media_id}`).then((res) => res.json()),
    });
    ...
    const handleClick = (event) => {
        fetch(`https://plugged-in-server.onrender.com/giveReward?id=${task?._id}`, {
            method: "PUT",
            headers: {
    ...
    return (
        <div className={`collapse ${media?.teamName !== team?.name ? "hidden" : ""}`}>
