...
const RequestCard = ({ request, connectionRequestRefetch }) => {
    ...
    return (
        <div className="card rounded-lg card-compact bg-base-100 shadow-xl">
            <Image className="w-full rounded-t-lg" src={protoURL ? protoURL : NoPhoto} alt="user" width={200} height={200} />
            <div className="card-body">
                <h2 className="card-title">{displayName}</h2>
                <p>{email}</p>
                <div className="card-actions">
                    <button onClick={handleAcceptRequest} className="btn btn-primary w-full">
                        Accept
                    </button>
                    <button onClick={handleDeleteConnect} className="btn btn-active w-full">
                        Delete
                    </button>
                </div>
            </div>
