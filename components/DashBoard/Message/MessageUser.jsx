...
const MessageUser = ({ room, active }) => {
    const { members, messages, teamName, roomType } = room;
    const { user: auth_user } = useAuth();

    let otherUser;
    if (roomType === "single") {
        if (auth_user?.email === members[0]) {
            otherUser = members[1];
        } else {
            otherUser = members[0];
        }
    ...
    if (messages) {
        if (messages[messages?.length - 1].msgContent.length > 15) {
            lastMessage = messages[messages?.length - 1].msgContent?.slice(0, 10) + "...";
        } else {
            ...
            >
                ...
                <div className="w-full">
                    <div className="-mb-1 flex justify-between items-center">
                        <h3 className="ml-3 font-semibold">{roomType === "single" ? user?.name : teamName}</h3>
                        {messages && <p className="ml-3 mt-0.5 text-sm">{messages[messages?.length - 1].time}</p>}
                    </div>
