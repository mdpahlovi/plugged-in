...
const Checkout = () => {
  ...
  const handleRegisterTeam = ({ name, email }) => {
    setPlanLoading(true);
    const emailWithoutat = authUser?.email.replace("@", "");
    const date = `${new Date()}`;
    ...
    jwt_axios.post(`/team`, team).then((res) => {
      if (res.data.acknowledged) {
        jwt_axios
          .patch(
            `/user/${email}`,
            user_obj(query?.plan, email, name, user?.team)
          )
          .then((res) => {
            const roomInfo = {
              roomName: `${emailWithoutat},${DateJoin}`
    ...
    <Main title="Checkout - PluggedIn">
      <Header>
        ...
        <p className="mt-2 max-w-xl px-6">
          Complete your order by providing your payment details. If you have any
          query please contact us. Thank you for choosing us.
        </p>
      ...
      <div className="container grid lg:grid-cols-2 gap-4 items-center">
        ...
        <form
          onSubmit={handleSubmit(handleRegisterTeam)}
          ...
          <div className="">
            ...
            <div className="relative">
              <input
                ...
                {...register("name")}
                className="pl-10 input"
                defaultValue={query?.plan === "team" ? "" : user?.name}
                required
              />
              <div className="pointer-events-none absolute inset-y-0 left-0 inline-flex items-center px-3">
                <HiOutlineUserCircle />
