...
const RecordDetails = () => {
    ...
    const {
        ...
        queryFn: () => fetch(`https://plugged-in-server.onrender.com/media/${query?.id}`).then((res) => res.json()),
    });
    ...
    const {
        ...
        queryFn: () => fetch(`https://plugged-in-server.onrender.com/tasks/${query?.id}`).then((res) => res.json()),
    });
