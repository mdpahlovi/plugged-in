...
const Recordings = () => {
    ...
    const {
        ...
        queryFn: () => fetch(`https://plugged-in-server.onrender.com/userMedia?email=${user?.email}`).then((res) => res.json()),
    });
                ...
                {isLoading
                    ...
                    : medias.map((media) => (
                          <MediaCard key={media._id} media={media} refetch={refetch} setShareMedia={setShareMedia} setDeletingRecordId={setDeletingRecordId}>
                              <Link href={user?.role === "basic" ? "/price" : `/dashboard/recordings/${media._id}`}>
                                  <ButtonOutline>
