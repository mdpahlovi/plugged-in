            ...
            jwt_axios.patch(`/team/${query?.leader}`, { members: [...restMembers] }).then((res) => {
                if (res.data.acknowledged) {
                    jwt_axios
                        ...
                        .then((res) => {
                            if (res.data) {
                                fetch(`https://plugged-in-server.onrender.com/deleteRoomMate`, {
                                    method: "PUT",
                                    headers: {
                                        "content-type": "application/json",
                                    },
    ...
    if (userLoading || teamLoading) {
        ...
        return (
            <Teams title="Add Member">
                ...
                <div className="mt-6 grid xs:grid-cols-[repeat(auto-fill,_minmax(23rem,_1fr))] gap-6">
                ...
                <TaskDetailsModal team={team} selectedMemberTask={selectedMemberTask} setSelectedMemberTask={setSelectedMemberTask} />
                <SeeRewardModal selectedMemberTask={seerewardMember} setSelectedMemberTask={setSeerewardMember} />
                <GiveRewardModal
